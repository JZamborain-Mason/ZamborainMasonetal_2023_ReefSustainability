...
parameters {
 vector[7] beta; //slopes of explanatory variables 
...
transformed parameters {
 ...
 for (i in 1:N) {
  bmin_r[i]=log(bmin)+beta[5] * op[i] +beta[6]*d[i]+beta[7]*hc[i];
  k[i] = log(B0) +beta[1] * op[i] +beta[2]*d[i]+beta[3]*hc[i];
  mu[i] = log(exp(k[i])/(1+((exp(k[i])-exp(bmin_r[i]))/exp(bmin_r[i]))*exp(-(r-p)*ag[i])))+ beta[4]*si[i]+u[pr[i]];
 }
 //remote submodel
  for(i in 1:R2){
