data {
 ...
 parameters {
   ...
   real log_p;// proportion of biomass exported
 }
 transformed parameters {
   ...
   real<lower=0> bmin; 
   real<lower=0> B0; 
   real<lower=0, upper=r> p; //export rate
   r=exp(log_r); 
   bmin=exp(log_bmin); 
   B0=exp(log_B0);  
   p=exp(log_p); 
   ...
   for (i in 1:res){ 
     k[i] = exp(log(B0) +beta[1] * op[i]+beta[2] * sst[i] +beta[3]*hc[i]+beta[4]*at[i]); 
        mu[i] = log(exp((r*log(k[i]) - p + exp(-r*ag[i])*(p - r*log(k[i]) + r*log(bmin)))/r))+beta[12]*d[i]+beta[5]*rh_c[i]+beta[6]*rh_f[i]+beta[7]*rh_b[i]+beta[8]*cm_pc[i]+beta[9]*sa[i]+ beta[10]*si[i];
    }
   //remote component
   for (i in 1:rem){ 
 ...
 model {
   ...
   log_p~ normal (-4,1); // uninformative prior for export proportion
   //likelihoods  
   for(n in 1:res){
