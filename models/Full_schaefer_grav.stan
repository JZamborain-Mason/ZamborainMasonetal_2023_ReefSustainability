data {
   ...
   real gr[res]; //predictor gravity (unstandardized so parameters are for 0 human pressure (no shifting baseline))
   //remote
   ...
   real gr3[fis]; //predictor gravity (unstandardized)
 ...
 transformed parameters {
   ...
   for (i in 1:res){ 
     ...
     mu[i] = log(exp(log(k[i]/(1+((k[i]-bmin)/bmin)*exp(-(r)*ag[i])))+beta[12]*d[i]+beta[5]*rh_c[i]+beta[6]*rh_f[i]+beta[7]*rh_b[i]+beta[8]*cm_pc[i]+beta[9]*sa[i]+ beta[10]*si[i]+ beta[13]*gr[i]));
   }
   //remote component
   for (i in 1:rem){ 
...
generated quantities {
   ...
   real site_Bmarg_all[res+rem+fis];// marginalize biomass by sampling and environmental conditions (to make ecosystem metrics comparable)
   real site_status[res+rem+fis]; //site-specific biomass status
   vector[res+rem+fis] log_lik; //log-likelihood 
   for (n in 1:res) {
     ...
     site_Bmarg_all[n] = exp(b[n]-(beta[1] * op[n] +beta[2] * sst[n]+beta[3]*hc[n]+beta[4]*at[n]+beta[12]*d[n]+beta[5]*rh_c[n]+beta[6]*rh_f[n]+beta[7]*rh_b[n]+beta[8]*cm_pc[n]+beta[9]*sa[n]+beta[13]*gr[n]));
   ...
   for (n in (res+rem+1):(res+rem+fis)) {
     ...
     site_Bmarg_all[n] = exp(b3[n-(res+rem)]-(beta[1] * op3[n-(res+rem)] +beta[2] * sst3[n-(res+rem)] +beta[3]*hc3[n-(res+rem)]+beta[4]*at3[n-(res+rem)]+beta[12]*d3[n-(res+rem)]+beta[8]*cm_pc3[n-(res+rem)]+beta[9]*sa3[n-(res+rem)]+beta[11]*cm_ds3[n-(res+rem)]+beta[5]*rh_c3[n-(res+rem)]+beta[6]*rh_f3[n-(res+rem)]+beta[7]*rh_b3[n-(res+rem)]+beta[13]*gr3[n-(res+rem)]));
   ...
   //average and common conditions
     BMMSY=B0/2;
     MMSY=((r*B0)/4); 
   //predicted biomass in reserves following logistic growth for average/common categories
   for (i in 1:res) {
     predreserveB[i] =  exp(log(B0/(1+(( B0-bmin)/bmin)*exp(-r*newage[i]))));
   }
