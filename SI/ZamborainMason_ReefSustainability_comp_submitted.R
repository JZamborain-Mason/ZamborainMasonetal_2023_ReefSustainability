...
#import McClanahan and Graham's 2015 data, kindly shared to me by Tim McClanahan (please ask for permission)
...
#import MacNeil et al. 2015 data (and code used) kindly shared to me by Aaron MacNeil (available on his github page)
#follow Aarons code 
 Adata<-read.csv("B0_master.csv", head=T)
  ...
  ggplot(data=aaronsites[aaronsites$protection=="Closed",], aes(x=mpage, y=exp(Aaronlbiomass)/10))+geom_point()+theme_classic()+geom_smooth(method="gam")+ylab("Total Biomass (t/km2)")+ggtitle("MacNeil et al. 2015 Original (arithmetic)")+ xlab ("Closure age")
...
#comparison of remote locations included
...
windows()
ggarrange(mombasa_trend,plot(Fit_null_p_mombasa,pars="B0"),plot(Fit_null_p_mombasa,pars="r"),plot(Fit_null_p_mombasa,pars="p"),plot(Fit_null_p_mombasa,pars="bmin"),ncol=5,nrow=1)
...
windows()
ggarrange(mombasa_trend,plot(Fit_null_p_mombasa,pars="B0")+geom_point(aes(x=)),plot(Fit_null_p_mombasa,pars="r"),plot(Fit_null_p_mombasa,pars="p"),plot(Fit_null_p_mombasa,pars="bmin"),ncol=5,nrow=1)
...
median(rstan::extract(Fit_null_p_mombasa_rem,par="bmin")$bmin)
...
#example (with Mcclanahan and Graham paper)
