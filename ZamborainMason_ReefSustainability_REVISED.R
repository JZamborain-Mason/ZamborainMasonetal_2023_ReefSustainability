...
#first read Aarons data (from MacNeil et al. 2015) to reclasify those he had defined as remote but that were <20h from human settlements
...
windows()
pairs(Fit_null_ss,pars=c("r","B0","bmin"))
...
ggarrange(a,b,nrow=1,ncol=2,widths = c(3,1))
...
b<-ggplot()+geom_jitter(data=extracteddata,aes(y=site_surplusrelMMSY,x=definedprotection,fill=as.factor(!extracteddata$site_surplusrelMMSY<0.8),color=as.factor(!extracteddata$site_surplusrelMMSY<0.8)),pch=21,alpha=0.2)+geom_violin(data=extracteddata,aes(y=site_surplusrelMMSY,x=definedprotection ),scale="width",fill="gray",col="darkgray",draw_quantiles = c( 0.5),alpha=0.5,lwd=1.2)+ 
  geom_text(aes(x=c("Openly fished","Restricted"),y=rep(1.08,2)),col="lightseagreen",lwd=5,label=paste(round(perc_atPGMSY*100),"%"))+
...
potential (surplus/MMSY)")+
  ...
  scale_color_manual(values = c("FALSE" = "gray",
                     ...
                     labels = c("FALSE" ="at PGMSY", 
                                "TRUE" ="belowBMMSY"))+guides(col=F,fill=F)+theme(axis.text = element_text(family="Helvetica",size=10,color="black"),axis.title = element_text(family="Helvetica",size=12),panel.background = element_rect(fill="white",colour="black"))+scale_y_continuous(breaks=c(0,0.8,1), limits=c(-0.05,1.08))
...
fig1<-ggarrange(fig1a,fig1b,nrow=1,ncol=2,widths=c(1,2))
annotate_figure(fig1,left="Posterior density")
...
#model without gravity and exports parametrized different ways
...
#######################################################################################################################################
#save.image(file='ZamborainMasonetal_2022_ReefSustainability_gravGF.RData')
