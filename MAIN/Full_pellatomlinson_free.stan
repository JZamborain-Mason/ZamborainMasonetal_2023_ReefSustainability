...
parameters {
 ...
 real pe;//parameter that modifies at which biomass the surplus production peaks (2 is graham-schaefer, 1 is gompertz-fox)
}
...
for (i in 1:res){ 
       ...
       mu[i] = log((1-p)*exp(log((k[i]^(1-pe)+(bmin^(1-pe)-k[i]^(1-pe))*exp(((y*m_res[i])/k[i])*(1-pe)*ag[i]))^(1/(1-pe)))+beta[12]*d[i]+beta[5]*rh_c[i]+beta[6]*rh_f[i]+beta[7]*rh_b[i]+beta[8]*cm_pc[i]+beta[9]*sa[i]+ beta[10]*si[i]));
    }
//remote component
for (i in 1:rem){ 
