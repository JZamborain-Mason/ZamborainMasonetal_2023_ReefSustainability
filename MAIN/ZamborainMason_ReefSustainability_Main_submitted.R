...
#number of sites sampled per jurisdiction
...
a<-ggplot(reefscale_data,aes(fill=Management))+geom_jitter(aes(x=Management,y=sHardCoral),alpha=0.5,pch=21)+geom_boxplot(aes(x=Management,y=sHardCoral),alpha=0.3,draw_quantiles = c( 0.3))+ylab("Std. hard coral cover")+xlab("")+guides(fill=F)+theme_classic()+
  scale_fill_manual (name="",values=c( "Fished"="red","Restricted"="turquoise1","UnfishedHigh"="yellow", "Remote"="green"))
b<-ggplot(reefscale_data,aes(fill=Management))+geom_jitter(aes(x=Management,y=sOcean_prod),alpha=0.5,pch=21)+geom_boxplot(aes(x=Management,y=sOcean_prod),alpha=0.8,draw_quantiles = c( 0.3))+ylab("Std. ocean productivity")+xlab("")+guides(fill=F)+theme_classic()+
  scale_fill_manual (name="",values=c( "Fished"="red","Restricted"="turquoise1","UnfishedHigh"="yellow", "Remote"="green"))
c<-ggplot(reefscale_data,aes(fill=Management))+geom_jitter(aes(x=Management,y=sSST),alpha=0.5,pch=21)+geom_boxplot(aes(x=Management,y=sSST),alpha=0.8,draw_quantiles = c( 0.3))+ylab("Std. SST")+xlab("")+guides(fill=F)+theme_classic()+
  scale_fill_manual (name="",values=c( "Fished"="red","Restricted"="turquoise1","UnfishedHigh"="yellow", "Remote"="green"))
...
#null model
Fit_null_ss<- stan(file = "Null_schaefer_ponb_review_logbmin.stan", data = stanDat_full_country, iter=20000,warmup=10000,thin=10,chains = 4,control = list(adapt_delta = 0.9))

#full model
Fit_full_ss<- stan(file = "Full_schaefer_nreref_logbmin.stan", data = stanDat_full_country, iter=20000,warmup=10000,thin=10,chains = 4,control = list(adapt_delta = 0.9))
...
#Simulate data to show the estimated effect of environmental covariates on reference points (assuming all other covaruates at average values)
#coral cover 
...
#bmin vs high impacted reefs
quantile(ordereddata$Grav_tot,0.75)
log(fished_data$FamBiomass_tkm2[fished_data$Grav_tot>quantile(ordereddata$Grav_tot,0.75) &fished_data$definedprotection=="Openly fished"])
windows()
ggplot()+geom_density(aes(x=list_of_draws_full$log_bmin),fill="darkred",alpha=0.5)+
  geom_density(aes(x=log(fished_data$FamBiomass_tkm2[fished_data$Grav_tot>quantile(ordereddata$Grav_tot,0.75) &fished_data$definedprotection=="Openly fished"])),fill="grey",alpha=0.5)+theme_classic()+xlab ("log-Biomass (t/km2))")+
  geom_text(aes(x=4,y=0.75),label="Bmin",col="darkred")+geom_text(aes(x=6,y=0.4),label="Openly fished sites
in high human impact",col="grey")
...
#status fig:sites open to extraction (excluding those in high compliance reserves)
...
#high grvaity locations have different environmental conditions and thus B0 (ongoing human induced environmental change?)
...
#F/fmsy patterns: 
fisheddata2$f_fmmsy<-fisheddata2$site_fstatus/fisheddata2$site_status
...
#check patterns
windows()
ggplot()+geom_point(aes(x=fixef(model_bstatus2)[-1,1],y=rownames(fixef(model_bstatus2))[-1]))+geom_errorbar(aes(x=fixef(model_bstatus2)[-1,1],y=rownames(fixef(model_bstatus2))[-1],xmin=fixef(model_bstatus2)[-1,3],xmax=fixef(model_bstatus2)[-1,4]))+theme_classic()+xlab("Effect size on Bweighted/BMMSY")+ylab("")+geom_vline(xintercept = 0,lty=2)
...
Fit_full_ss_closed<- stan(file = "Full_schaefer_nreref_logbmin_closed.stan", data = stanDat_full_country, iter=20000,warmup=10000,thin=10,chains = 4,control = list(adapt_delta = 0.9))
...
Fit_full_ss_nre<- stan(file = "Full_schaefer_nreref_noremote.stan", data = stanDat_full_country, iter=20000,warmup=10000,thin=10,chains = 4,control = list(adapt_delta = 0.9))
...
Fit_full_pt<- stan(file = "Full_pellatomlinson_free_noreref.stan", data = stanDat_full_country, iter=20000,warmup=10000,thin=10,chains = 4,control = list(adapt_delta = 0.9))
...
#gompertz fox
Fit_full_gf<- stan(file = "Full_gompertzfox_noreref.stan", data = stanDat_full_country, iter=20000,warmup=10000,thin=10,chains = 4,control = list(adapt_delta = 0.99))
Bio<- seq(0,350,1) #biomass data for surplus production curve
B<- length(Bio)
...
stanDat_full_country4<- list(b = log(reserves_complete$FamBiomass_tkm2),ag=reserves_complete$Closure.age,
                             ...
                             newage=newdata$Closure.age,
                             Bio=Bio, B=B,pe=4)
Fit_full_pt4<- stan(file = "Full_pellatomlinson_fixed_noreref.stan", data = stanDat_full_country4, iter=20000,warmup=10000,thin=10,chains = 4,control = list(adapt_delta = 0.99))
